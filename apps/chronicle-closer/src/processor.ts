...
class ChronicleClosureProcessor {
  ...
  async process(event: ChronicleClosureEvent): Promise<void> {
    const snapshot = await this.#loadSnapshot(event);
  ...
  async #loadSnapshot(event: ChronicleClosureEvent): Promise<ChronicleSnapshot> {
  ...
  #buildContext(snapshot: ChronicleSnapshot): SummaryContext {
    ...
    return {
      ...
      skillHighlights,
      threadLines: buildThreadLines(snapshot.chronicle),
      transcript,
    };
  }
