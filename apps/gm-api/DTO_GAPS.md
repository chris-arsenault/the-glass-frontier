...
- **Intent fields**: The nodes depend on `intentType`, `beatDirective`, `creativeSpark`,
  `routerConfidence`, and `routerRationale`, but the v2 `IntentSchema` only exposes basic tone/skill
...
- Location nodes rely on:
  - Fetching the complete location graph + prior `LocationState`.
