...
// Lazy getters for world state - initialized on first request
const getWorld = () => getWorldState().world;
const getChronicles = () => getWorldState().chronicles;
...
app.post('/chronicles', async (req, res) => {
  try {
    ...
    const chronicle = await chronicles.ensureChronicle({
