...
|----------|------|------------|-------------------|-------------------|----------------------|
| P1 | Tier 1 | Gameplay implementation (hub contests + offline pipeline) | IMP-HUBS-05, IMP-OFFLINE-05 | Stage CLI artefacts refreshed (contest summary 2025-11-04T10:17Z, offline QA drift rollup 2025-11-04T10:13Z); escalate staging Temporal + storage access to gather live telemetry for SME distribution. | Requires staging Temporal + storage access; outputs must satisfy DES-BENCH-01 and DES-16. |
| P1 | Tier 1a | Unified web client overlays | IMP-CLIENT-06 | Continue stage telemetry runs (`npm run stage:smoke`, `npm run stage:alerts`); latest pass (2025-11-04T10:12Z) healthy—publish metrics and chase SME approval in #client-overlays / #admin-sse once staging storage returns. | Depends on IMP-OFFLINE-05 drift telemetry and contest data to display retry/alert info. |
