...
Historical product intent, retained from the original repository README on
2026-09-21. This document includes proposed systems and obsolete delivery
processes; it is not a description of the deployed application or an active
instruction to use the former MCP backlog. See the [current architecture](../architecture.md)
and [active backlog](../../BACKLOG.md) for implementation status and follow-up work.
...
- Permanently avoid planning, grooming, or executing PBIs that revolve around CI pipelines, deployment automation, reminder bots, artefact packaging, or other non-gameplay side processes—close or reject them immediately.
