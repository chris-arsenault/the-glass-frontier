...
- Moderation dashboard now surfaces aggregated contest sentiment telemetry, including cooldown spike counts, negative hotspots, and the latest samples, via `/admin/moderation/contest/sentiment`.
- Trimmed rematch cooldown defaults (risk-it-all 9 s, sparring 6 s, clash-of-wills 11 s) and softened timeout momentum penalties, with moderation dashboards now highlighting cooldown frustration ratios when sentiment spikes.
...
- `src/hub/commandParser.js` enriches command metadata with contest descriptors (contest key, participants, contested actors) so downstream narrators/telemetry/alarm paths stay aware of PvP escalations, and now normalises verb-level rematch cooldown specs.
- `src/hub/orchestrator/contestCoordinator.js` orchestrates contest bundling, timeout expiry, and Temporal workflow hand-offs, tracking rematch cooldown windows so duplicate call-outs are deferred until the configured wait elapses.
- Timeout fallout now targets hesitation roles: challengers avoid momentum loss on single-miss stalls, allied/support roles only drop momentum on severe deficits, and moderation cues surface frustration ratios to aid cooldown tuning.
...
- `ContestCoordinator.expire` runs a timeout outcome simulator that scales shared complication severity and momentum penalties based on missing participants so multi-actor arming deficits surface consistent fallout.
...
- `scripts/benchmarks/contestWorkflowMonitor.js` parses `telemetry.contest.sentiment` events, aggregates cooldown spikes, and exposes hotspots/latest samples so moderation surfaces can highlight frustration spikes.
- `src/moderation/moderationService.js` exposes `getContestSentimentOverview` for the moderation router, and `/admin/moderation/contest/sentiment` feeds the updated dashboard cards.
...
- Added integration coverage for contest timeouts and rematch cooldown gating, ensuring `contestExpired` state updates broadcast before subsequent commands and rematch attempts respect cooldowns.
- Expanded `__tests__/client/components.test.jsx` so contested encounters—including timeout narration and rematch cooldown prompts—surface in the Check Overlay with screen-reader-friendly copy.
- Added unit coverage for `ContestMetrics` (`__tests__/unit/telemetry/contestMetrics.test.js`), the contest monitoring CLI (`__tests__/unit/scripts/contestWorkflowMonitor.test.js`), the ContestCoordinator timeout workflow, and the new rematch enforcement path to keep latency analysis and anti-spam logic deterministic.
