...
- Instrumented Playwright regression coverage (`tests/e2e/admin-moderation.spec.js`) exercising the DES-18 role guard, live alert review, and approve decision flow to lock the admin experience.
...
- Data-test attributes on moderation UI controls allow deterministic automation while satisfying DES-18 transparency requirements for SMEs.

## Verification
- `npm test` — Jest suite covering moderation service unit tests and moderation route integrations, alongside existing coverage.
- `npx playwright test tests/e2e/admin-moderation.spec.js` — validates moderator login, alert review, and approval workflow over authenticated UI.
