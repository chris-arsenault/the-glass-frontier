...
const log = (...args: unknown[]) => {
  console.log('[seed-local-fixtures]', ...args);
};
...
async function main(): Promise<void> {
  ...
  await resetPlaywrightFixtures(worldstateDatabaseUrl);
...
main().catch((error) => {
  console.error('[seed-local-fixtures] Failed', error);
  process.exitCode = 1;
});
