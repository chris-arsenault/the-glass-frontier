...
4. **Structure with features:** Treat features as the parent container for initiatives spanning multiple PBIs. Before creating new work, confirm whether an existing feature applies (`mcp__game-mcp-server__list_features`, `mcp__game-mcp-server__list_feature_backlog_items`) or establish a new one (`mcp__game-mcp-server__create_feature`, `mcp__game-mcp-server__update_feature`). Link each backlog item using `mcp__game-mcp-server__assign_backlog_to_feature`.
