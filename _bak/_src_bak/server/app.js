...
import { createAdminHubVerbRouter  } from "./routes/adminHubVerbs.js";
import { AccountService  } from "../auth/accountService.js";
import { SessionDirectory  } from "../auth/sessionDirectory.js";
import { PublishingCadence  } from "../offline/publishing/publishingCadence.js";
...
import { SessionStatusEvent, PlayerControl  } from "../../_lib_bak/envelopes/index.js";
...
function createApp({
  ...
  app.post("/sessions/:sessionId/close", authenticate, (req, res, next) => {
    ...
    try {
      ...
      const sessionClosedEvent = SessionStatusEvent.closed(summary.updatedAt, auditRef);
      const closedPayload = {
        ...sessionClosedEvent.serialize(),
        reason: reason || "session.closed",
        momentum: sessionState.momentum || {},
        pendingChecks: pendingChecks.length
      };

      broadcaster.publish(sessionId, closedPayload);
