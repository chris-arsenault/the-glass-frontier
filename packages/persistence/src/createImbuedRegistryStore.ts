...
const normalizeBucket = (explicit?: string | null): string => {
  const candidate = explicit ?? process.env.WORLD_STATE_S3_BUCKET ?? null;
  if (candidate !== null && candidate.trim().length > 0) {
    return candidate;
  }
  throw new Error('Imbued registry store requires WORLD_STATE_S3_BUCKET to be configured.');
};

const resolvePrefix = (explicit?: string | null): string | undefined => {
  const candidate = explicit ?? process.env.WORLD_STATE_S3_PREFIX ?? null;
