...
import { ContestCoordinator  } from "./contestCoordinator.js";
import { classifySentiment, inferTone  } from "../../utils/sentiment.js";
import { HubStateEvent  } from "../../../_lib_bak/envelopes/index.js";
...
class HubOrchestrator extends EventEmitter {
  ...
  async _handleConnectionOpened(connection) {
    ...
    const stateEvent = HubStateEvent.snapshot(
      connection.hubId,
      connection.roomId,
      snapshot.state,
      snapshot.version
    );
    const message = stateEvent.serialize();
  ...
  _broadcastState({ hubId, roomId, state, version, command = null, workflow = null, meta = {} }) {
    ...
    const stateEvent = new HubStateEvent({
    ...
    this.gateway.broadcastToRoom(roomId, stateEvent.serialize());
  }
